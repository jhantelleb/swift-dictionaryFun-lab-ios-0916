...
func replaceFlavor(flavor: String, forPerson person: String) -> Bool {
    ...
    for (name, flavors) in favoriteFlavorsOfIceCream{
        if name == person{
            let value = favoriteFlavorsOfIceCream.updateValue(flavor, forKey: person)
            if value != nil{ //person exists and the flavor has been changed
                return true
            }else {
                return false
            }
...
func removePerson(name: String) -> Bool {
    // write your code here
    var flag: Bool = true
    for (removeName, flavor) in favoriteFlavorsOfIceCream{
        if removeName == name{
          let removed = favoriteFlavorsOfIceCream.removeValueForKey(name)
            if removed != nil {
                flag = true
            }else{
                flag = false
...
func addPerson(name: String, withFlavor flavor: String) -> Bool {
    // write your code here
    let addEntry = favoriteFlavorsOfIceCream.updateValue(flavor, forKey: name)
    if addEntry != nil{
       return false
    }else{
        return true
...
iceCream = iceCreamForPerson("Mark")
print("\(iceCream)")
print("\(favoriteFlavorsOfIceCream)")
if let iceCream = iceCream { //Shouldn't this be chocolate since you changed Mark's Ice cream to Chocolate in line 266?
    //assert(iceCream == "Cookies 'N' Cream", "\(iceCream)")
    assert(iceCream == "Chocolate", "\(iceCream)")
} else {
    assert(false, "\(iceCream)")
}
